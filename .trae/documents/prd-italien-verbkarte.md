## 1. Produktüberblick
Eine webbasierte Forschungs-App zur gemeinsamen Erfassung, Pflege und Visualisierung linguistischer Befunde auf einer interaktiven Italien-Karte.
- Das Produkt unterstützt Forschende dabei, Städte als Datenpunkte gemeinsam zu verwalten und pro Ort zu dokumentieren, ob relevante Verben mit `ar-` statt `re-` oder `ri-` beginnen.
- Der fachliche Mehrwert liegt in einer zentralen, filterbaren und exportierbaren Datengrundlage für räumliche Analysen, Kartenansichten und spätere Weiterverarbeitung in Tabellenkalkulationen.
...
5. **Datenexport und Persistenz**: Zentrale Speicherung in Supabase sowie Export nach `CSV` und `XLSX`.
...
| Hauptansicht | Import/Export | Exportiert alle Datensätze nach `CSV` oder `XLSX` und bereitet spätere strukturierte Importe vor. |
| Hauptansicht | Gemeinsame Datenhaltung | Lädt den zentralen Datenbestand aus Supabase und speichert neue oder bearbeitete Befunde sofort fuer alle Beteiligten. |

## 3. Kernablauf
Die Nutzerin öffnet die Karte, lädt den gemeinsamen Datenbestand aus Supabase, sucht eine Stadt oder navigiert auf der Karte zu einem Ort und wählt den zu bearbeitenden Punkt aus. Danach erfasst oder aktualisiert sie den linguistischen Befund inklusive Kategorie, Beleg und Quelle. Die Markerfarbe, die Tabelle und die Statistik werden sofort aktualisiert. Über Filter kann die Nutzerin räumliche Verteilungen gezielt untersuchen und die Datensätze für die weitere Auswertung exportieren.

```mermaid
flowchart TD
    A["App starten"] --> B["Karte und Supabase-Daten laden"]
    B --> C["Ort suchen oder Marker auswählen"]
    C --> D["Befundformular öffnen"]
    D --> E["Kategorie und Metadaten erfassen"]
    E --> F["Datensatz in Supabase speichern"]
...
| Hauptansicht | Synchronisationsstatus | Hinweis auf Ladevorgang, Speicherstatus und eventuelle Fehler der zentralen Datenhaltung |
